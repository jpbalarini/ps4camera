...
int USBMgr::listDevices(std::vector<PS4EYECam::PS4EYERef> &list) {
  ...
  while ((dev = devs[i++]) != NULL) {
    ...
    if (desc.idVendor == PS4EYECam::VENDOR_ID &&
        desc.idProduct == PS4EYECam::PRODUCT_ID) {
...
public:
  URBDesc(uint mode)
      : num_transfers(0), last_packet_type(DISCARD_PACKET), last_pts(0),
        last_fid(0) {
    is_streaming = false;
  ...
  URBDesc()
    ...
    const size_t fsz =
        5571968; // hardcoded interface dwMaxVideoFrameBufferSize 5571968
  ...
  bool start_transfers(libusb_device_handle *handle, uint32_t curr_frame_size) {
    ...
    for (i = 0; i < 8; i++) {
      ...
      libusb_fill_iso_transfer(xfr[i], handle, 0x81,
                               frame_buffer_end + 49152 * 8 * i, 49152 * 8, 8,
                               cb_xfr, reinterpret_cast<void *>(this), 1000);
  ...
  void frame_add(enum gspca_packet_type packet_type, const uint8_t *data,
                 int len) {
    int i;
...
const std::vector<PS4EYECam::PS4EYERef> &
PS4EYECam::getDevices(bool forceRefresh) {
...
bool PS4EYECam::updateDevices() { return USBMgr::instance()->handleEvents(); }
...
PS4EYECam::PS4EYECam(libusb_device *device, bool firmware_check) {
  ...
  // create a usb control trasfer packet. This is used to send commands to the
  // device.
...
void PS4EYECam::release() {
  ...
  if (handle_ != NULL)
    close_usb();
  if (control_transfer_buffer)
    free(control_transfer_buffer);
  if (myframe.unknown1)
    free(myframe.unknown1);
  if (myframe.unknown2)
    free(myframe.unknown2);
  if (myframe.videoLeftFrame)
    free(myframe.videoLeftFrame);
  if (myframe.videoRightFrame)
    free(myframe.videoRightFrame);
  if (myframe.unknown3)
    free(myframe.unknown3);
}
...
void PS4EYECam::multia7_register_write(const uint16_t data[][3], uint8_t len) {
  ...
  while (i <= len * 4 + 16 - 4) {
    ...
    debug("%i 0x%02x 0x%02x 0x%02x 0x%02x \n", i, buffer_out[i],
          buffer_out[i + 1], buffer_out[i + 2], buffer_out[i + 3]);
    i = i + 4;
  }
  /*Send register to write to subaddr*/
  submitAndWait_controlTransfer(LIBUSB_ENDPOINT_OUT |
 ...
 */
void PS4EYECam::multi_register_write(const uint16_t data[][2], uint8_t len,
                                     uint8_t subaddr) {
  ...
  case OV9713_SENSOR2_SUBADDR: /* for writing sensor2 only?*/
    buffer_out[8] = 0x03;
    break;
  case OV9713_SENSORS_SUBADDR: /*it seems that 0x03 is for writing registers in
                                  both image sensors at the same time*/
    buffer_out[8] = 0x03;
    break;
...
/*
 * Dump multiple registers from start address to finish address of the chips
 * managed by ov580 start begin address finish end address subaddr to choose
 * right device
 */
void PS4EYECam::dump_registers(uint16_t start, uint16_t finish,
                               uint8_t subaddr) {
 ...
 * subaddr to choose right device
 */
void PS4EYECam::multi_register_read(const uint16_t(*data), uint8_t len,
                                    uint8_t subaddr) {
...
void PS4EYECam::register_write(uint16_t reg, uint8_t val, uint8_t subaddr) {
  ...
  submitAndWait_controlTransfer(LIBUSB_ENDPOINT_OUT |
                                    LIBUSB_REQUEST_TYPE_VENDOR |
                                    LIBUSB_RECIPIENT_DEVICE,
                                SET_REGISTER, 0, 0, 0X40, buffer_out);
}

/*
 * Read a register from one of the chips managed by ov580
 * Read in done in 2 phases, first we send reg to read and subaddr with brequest
 * 0xa5 then in second phase with brequest 0xa6 we received value from register
...
void PS4EYECam::dump_sensor_generalconf(uint8_t n) {
  ...
  multi_register_read(sensor_generalconf_imagesize_reg_read,
                      ARRAY_SIZE(sensor_generalconf_imagesize_reg_read), n);
}
/*
...
void PS4EYECam::dump_sensor_video_mode(uint8_t n) {
  ...
  multi_register_read(ov9713_video_mode_registers,
                      ARRAY_SIZE(ov9713_video_mode_registers), n);
}
/*
...
void PS4EYECam::set_sensor_video_mode(uint8_t mode, uint8_t n) {
  ...
  switch (mode) {
  case 0:
    multi_register_write(ov9713_video_mode_0, ARRAY_SIZE(ov9713_video_mode_0),
                         n);
    break;
  case 1:
    multi_register_write(ov9713_video_mode_1, ARRAY_SIZE(ov9713_video_mode_1),
                         n);
    break;
  case 2:
    multi_register_write(ov9713_video_mode_2, ARRAY_SIZE(ov9713_video_mode_2),
                         n);
    break;
  case 3:
    multi_register_write(ov9713_video_mode_3, ARRAY_SIZE(ov9713_video_mode_3),
                         n);
    break;
  case 4:
    multi_register_write(ov9713_video_mode_4, ARRAY_SIZE(ov9713_video_mode_4),
                         n);
    break;
  case 5:
    multi_register_write(ov9713_video_mode_5, ARRAY_SIZE(ov9713_video_mode_5),
                         n);
    break;
  case 6:
    multi_register_write(ov9713_video_mode_6, ARRAY_SIZE(ov9713_video_mode_6),
                         n);
    break;
  case 7:
    multi_register_write(ov9713_video_mode_7, ARRAY_SIZE(ov9713_video_mode_7),
                         n);
    break;
  case 8:
    multi_register_write(ov9713_video_mode_8, ARRAY_SIZE(ov9713_video_mode_8),
                         n);
    break;
  case 9:
    multi_register_write(ov9713_video_mode_9, ARRAY_SIZE(ov9713_video_mode_9),
                         n);
    break;
  case 10:
    multi_register_write(ov9713_video_mode_10, ARRAY_SIZE(ov9713_video_mode_10),
                         n);
    break;
  case 11:
    multi_register_write(ov9713_video_mode_11, ARRAY_SIZE(ov9713_video_mode_11),
                         n);
    break;
  case 12:
    multi_register_write(ov9713_video_mode_12, ARRAY_SIZE(ov9713_video_mode_12),
                         n);
    break;
  case 13:
    multi_register_write(ov9713_video_mode_13, ARRAY_SIZE(ov9713_video_mode_13),
                         n);
    break;
  case 14:
    multi_register_write(ov9713_video_mode_14, ARRAY_SIZE(ov9713_video_mode_14),
                         n);
    break;
  case 15:
    multi_register_write(ov9713_video_mode_15, ARRAY_SIZE(ov9713_video_mode_15),
                         n);
    break;
  case 16:
    multi_register_write(ov9713_video_mode_16, ARRAY_SIZE(ov9713_video_mode_16),
                         n);
    break;
  case 17:
    multi_register_write(ov9713_video_mode_17, ARRAY_SIZE(ov9713_video_mode_17),
                         n);
    break;
  case 18:
    multi_register_write(ov9713_video_mode_18, ARRAY_SIZE(ov9713_video_mode_18),
                         n);
    break;
  case 19:
    multi_register_write(ov9713_video_mode_19, ARRAY_SIZE(ov9713_video_mode_19),
                         n);
    break;
  case 20:
    multi_register_write(ov9713_video_mode_20, ARRAY_SIZE(ov9713_video_mode_20),
                         n);
    break;
  case 21:
    multi_register_write(ov9713_video_mode_21, ARRAY_SIZE(ov9713_video_mode_21),
                         n);
    break;
  case 22:
    multi_register_write(ov9713_video_mode_22, ARRAY_SIZE(ov9713_video_mode_22),
                         n);
    break;
  case 23:
    multi_register_write(ov9713_video_mode_23, ARRAY_SIZE(ov9713_video_mode_23),
                         n);
    break;
  case 24:
    multi_register_write(ov9713_video_mode_24, ARRAY_SIZE(ov9713_video_mode_24),
                         n);
    break;
  case 25:
    multi_register_write(ov9713_video_mode_25, ARRAY_SIZE(ov9713_video_mode_25),
                         n);
    break;
  case 26:
    multi_register_write(ov9713_video_mode_26, ARRAY_SIZE(ov9713_video_mode_26),
                         n);
    break;
  case 27:
    multi_register_write(ov9713_video_mode_27, ARRAY_SIZE(ov9713_video_mode_27),
                         n);
    break;
  case 28:
    multi_register_write(ov9713_video_mode_28, ARRAY_SIZE(ov9713_video_mode_28),
                         n);
    break;
  default:
...
void PS4EYECam::set_led_on() { register_write(0x2706, 0x01, 0xff); }
...
bool PS4EYECam::init_registers() {
  ...
  // setting ov9713 registers in sync mode both sensor at the same time
  // subaddres 0x03
  ...
  multi_register_write(unknow30_reg_init0, ARRAY_SIZE(unknow30_reg_init0),
                       0x30);
  ...
  // after write to ak5703 usb sniffer shows that it is reading these registers
  // perhaps sony 's driver is checking after writing i don't know :P
...
bool PS4EYECam::init(uint32_t width, uint32_t height,
                     uint8_t desiredFrameRate) {
  ...
  // Give sensors time to initialize after register setup
  debug("Waiting for sensors to initialize...\n");
  sleep(2);

  // Temporarily bypass sensor check since firmware might be needed
  debug("Bypassing sensor check for now (firmware may be required)\n");
  /*
  if (!check_sensors()) {
  ...
  */
  /*  TODO play with video modes perhaps we will need to go to uvc path without
     ov580 information. debug("Show video mode registers from sensor 1\n");
      dump_sensor_video_mode(1);
  ...
  // Try to initialize registers before sensor check
  ...
  debug("Register initialization completed\n");
  return true;
}
...
void PS4EYECam::check_ff71() {
  ...
  case 0:
    ...
    if (val == 0x0) {
      ...
      //   usleep(1);
      multi_register_write(ov580_reg_stream_w0, ARRAY_SIZE(ov580_reg_stream_w0),
                           0xff);
      urb->ff71status = 1;
    }
  ...
  case 1:
    ...
    multi_register_write(ov580_reg_stream_w1, ARRAY_SIZE(ov580_reg_stream_w1),
                         0xff);
    usleep(100);
    val = register_read(0xff71, 0xff);
    debug("i=%d val=%d\n", urb->ff71status, val);
...
eyeframe *PS4EYECam::getLastVideoFramePointer() {
  ...
  for (i = 0; i < frame_height; i++) {
    memcpy(&myframe.videoLeftFrame[frame_width * 2 * i],
           &rowpointer[linesize * 2 * i + 32 + 64], frame_width * 2);
    memcpy(&myframe.videoRightFrame[frame_width * 2 * i],
           &rowpointer[linesize * 2 * i + 32 + 64 + frame_width * 2],
           frame_width * 2);
  }
  return &myframe;
...
/*  const uint8_t* PS4EYECam::getLastVideoFramePointer()
  {
      ...
      uint8_t *rowpointer;
      //rowpointer=(urb->frame_buffer+ urb->frame_complete_ind *
  urb->frame_size); rowpointer=&(urb->frame_buffer[urb->frame_complete_ind *
  urb->frame_size]);
     ...
     // for(i=0;i<800;i++)
      ...
      for(i=0;i<frame_height;i++)
      {
       ...
       // memcpy(&myframe.videoframe[1280*2*i],&rowpointer[linesize*2*i+32+64],
  1280*2);
          memcpy(&myframe.videoLeftFrame[frame_width*2*i],&rowpointer[linesize*2*i+32+64],
  frame_width*2);


          //
  memcpy(&myframe.depthframe[1280*2*i],&rowpointer[linesize*2*i+32+64+1280*2],
  1280*2);
          //
  memcpy(&myframe.unknown3[840*2*i],&rowpointer[linesize*2*i+32+64+1280*2+1280*2],
  840*2);
  ...
  //}
  /*
   * dump fuctions from lsusb.c only for debug purposes to check if values are
  the same in
   * usb sniffer capture
   */
void PS4EYECam::dump_endpoint_comp(
...
void PS4EYECam::dump_endpoint(
  ...
  ret = libusb_get_ss_endpoint_companion_descriptor(
      USBMgr::instance()->usbContext(), endpoint, &ep_comp);
...
void PS4EYECam::dump_altsetting(
  ...
  debug("   Interface Descriptor:\n"
        ...
        interface->bNumEndpoints, interface->bInterfaceClass, /*cls,*/
        interface->bInterfaceSubClass,
        /*subcls,*/ interface->bInterfaceProtocol, /*proto,*/
  ...
  if (config->extra_length) {
    ...
    if (buf[1] == 0x0b) {
      debug("   Interface Association:\n"
            ...
            "     iFunction         %5u \n",
            buf[0], buf[1], buf[2], buf[3], buf[4], /*cls,*/
            buf[5],                                 /*subcls,*/
            buf[6],                                 /*proto,*/
            buf[7] /*, func*/);
    }
...
void PS4EYECam::dump_device() {
  ...
  debug("Device Descriptor:\n"
        ...
        descriptor.bLength, descriptor.bDescriptorType, descriptor.bcdUSB >> 8,
        descriptor.bcdUSB & 0xff, descriptor.bDeviceClass, /*cls,*/
        descriptor.bDeviceSubClass,                        /*subcls,*/
        descriptor.bDeviceProtocol,                        /*proto,*/
        descriptor.bMaxPacketSize0, descriptor.idVendor,
        /*vendor,*/ descriptor.idProduct, /*product,*/
...
int PS4EYECam::uvc_get_hue(int16_t *hue, uint8_t req_code) {
  ...
  ret = libusb_control_transfer(handle_, REQ_TYPE_GET, req_code,
                                UVC_PU_HUE_CONTROL << 8, 1 << 8, data,
                                sizeof(data), 1000);
...
int PS4EYECam::uvc_set_hue(int16_t hue) {
  ...
  ret = libusb_control_transfer(handle_, REQ_TYPE_SET, UVC_SET_CUR,
                                UVC_PU_HUE_CONTROL << 8, 1 << 8, data,
                                sizeof(data), 0);
...
int PS4EYECam::uvc_get_sharpness(uint16_t *sharpness, uint8_t req_code) {
  ...
  ret = libusb_control_transfer(handle_, REQ_TYPE_GET, req_code,
                                UVC_PU_SHARPNESS_CONTROL << 8, 1 << 8, data,
                                sizeof(data), 0);
...
int PS4EYECam::uvc_get_brightness(int16_t *brightness, uint8_t req_code) {
  ...
  ret = libusb_control_transfer(handle_, REQ_TYPE_GET, req_code,
                                UVC_PU_BRIGHTNESS_CONTROL << 8, 1 << 8, data,
                                sizeof(data), 0);
...
int PS4EYECam::uvc_get_contrast(uint16_t *contrast, uint8_t req_code) {
  ...
  ret = libusb_control_transfer(handle_, REQ_TYPE_GET, req_code,
                                UVC_PU_CONTRAST_CONTROL << 8, 1 << 8, data,
                                sizeof(data), 1000);
...
int PS4EYECam::uvc_get_saturation(uint16_t *saturation, uint8_t req_code) {
  ...
  ret = libusb_control_transfer(handle_, REQ_TYPE_GET, req_code,
                                UVC_PU_SATURATION_CONTROL << 8, 1 << 8, data,
                                sizeof(data), 0);
...
int PS4EYECam::uvc_get_white_balance_temperature(uint16_t *temperature,
                                                 uint8_t req_code) {
  uint8_t data[2];
  int ret;
...
int PS4EYECam::uvc_get_gain(uint16_t *gain, uint8_t req_code) {
  ...
  ret = libusb_control_transfer(handle_, REQ_TYPE_GET, req_code,
                                UVC_PU_GAIN_CONTROL << 8, 1 << 8, data,
                                sizeof(data), 0);
...
int PS4EYECam::uvc_get_power_line_frequency(uint8_t *power_line_frequency,
                                            uint8_t req_code) {
  uint8_t data[1];
  int ret;
...
int PS4EYECam::uvc_set_video_mode(uint8_t mode, uint8_t fps) {
  ...
  err = libusb_control_transfer(handle_, 0x21, 0x01, (0x01 << 8),
...
int PS4EYECam::uvc_show_video_mode() {
  ...
  /* do the transfer */
  err = libusb_control_transfer(handle_, 0xA1, 0x81, (1 << 8), 1, buf, len, 0);
  if (err == len) {
...
bool PS4EYECam::open_usb() {
  ...
  // Skip device reset as it may cause re-enumeration issues on macOS
  debug("Skipping USB device reset to avoid re-enumeration issues\n");
  ...
  // PS4 Eye camera needs both interface 0 and interface 1 claimed
  // Interface 1 is critical for video streaming (endpoint 0x81)
  int interface_to_use = 0;
  bool interface_claimed = false;

  for (int iface = 0; iface <= 1; iface++) {
    debug("Trying interface %d...\n", iface);

    // Check if kernel driver is active and detach it if necessary
    int kernel_active = libusb_kernel_driver_active(handle_, iface);
    if (kernel_active == 1) {
      debug("Kernel driver active on interface %d, attempting to detach...\n", iface);
      int detach_res = libusb_detach_kernel_driver(handle_, iface);
      if (detach_res != 0) {
        debug("Could not detach kernel driver from interface %d: %d (this may be due to macOS entitlements)\n", iface, detach_res);
        debug("Trying to claim interface anyway...\n");
      } else {
        debug("Kernel driver detached successfully from interface %d\n", iface);
      }
    } else if (kernel_active < 0) {
      debug("Error checking kernel driver status on interface %d: %d\n", iface, kernel_active);
    } else {
      debug("No kernel driver active on interface %d\n", iface);
    }

    // Try to claim the interface
    res = libusb_claim_interface(handle_, iface);
    if (res == 0) {
      debug("Successfully claimed interface %d\n", iface);
      if (iface == 0) {
        interface_to_use = iface;
        claimed_interface_ = iface;
        interface_claimed = true;
      }
      // For PS4 Eye, we need both interfaces, so don't break early
    } else {
      debug("Failed to claim interface %d: %d\n", iface, res);
      if (iface == 1) {
        debug("WARNING: Failed to claim interface 1 - video streaming may not work\n");
      }
    }
  }

  if (!interface_claimed) {
    debug("Could not claim interface 0\n");
    return false;
  }

  // set all setting 0 to interface (commented out)
  // res=libusb_set_interface_alt_setting(handle_, interface_to_use, 0);

  // PS4 Eye camera specifically needs interface 1 with alt setting 1 for video streaming
  // This is critical for enabling the isochronous endpoint 0x81
  debug("Setting interface 1 to alt setting 1 for video streaming...\n");
  res = libusb_set_interface_alt_setting(handle_, 1, 1);
  if (res == 0) {
    debug("Successfully set interface 1 to alt setting 1\n");
  } else {
    debug("Failed to set interface 1 to alt setting 1: %d\n", res);
    debug("This may cause endpoint 0x81 errors\n");
  }

  // Also try to set alt setting on the claimed interface as backup
  bool alt_setting_success = false;
  for (int alt_setting = 0; alt_setting <= 1 && !alt_setting_success; alt_setting++) {
    debug("Trying alt setting %d on interface %d...\n", alt_setting, interface_to_use);
    res = libusb_set_interface_alt_setting(handle_, interface_to_use, alt_setting);
    if (res == 0) {
      debug("Successfully set alt setting %d on interface %d\n", alt_setting, interface_to_use);
      alt_setting_success = true;
    } else {
      debug("Failed to set alt setting %d on interface %d: %d\n", alt_setting, interface_to_use, res);
    }
  }

  if (!alt_setting_success) {
    debug("Could not set any alt setting on interface %d, continuing anyway...\n", interface_to_use);
    // Don't return false here, some devices might work without alt settings
  }
  uvc_show_video_mode();
...
void PS4EYECam::close_usb() {
  ...
  if (handle_ != NULL) {
    // Release the claimed interface if we know which one it was
    if (claimed_interface_ >= 0) {
      debug("Releasing claimed interface %d\n", claimed_interface_);
      int release_res = libusb_release_interface(handle_, claimed_interface_);
      if (release_res == 0) {
        debug("Released interface %d\n", claimed_interface_);

        // Try to reattach kernel driver if it was detached
        if (libusb_kernel_driver_active(handle_, claimed_interface_) == 0) {
          debug("Reattaching kernel driver to interface %d\n", claimed_interface_);
          int attach_res = libusb_attach_kernel_driver(handle_, claimed_interface_);
          if (attach_res != 0) {
            debug("Could not reattach kernel driver to interface %d: %d\n", claimed_interface_, attach_res);
          } else {
            debug("Kernel driver reattached to interface %d\n", claimed_interface_);
          }
        }
      } else {
        debug("Failed to release interface %d: %d\n", claimed_interface_, release_res);
      }
    }
 ...
 * been processed.
 */
void PS4EYECam::submitAndWait_controlTransfer(uint8_t bmRequestType,
                                              uint8_t bRequest, uint16_t wValue,
                                              uint16_t wIndex, uint16_t wLength,
                                              uint8_t *buffer) {
  submit_controlTransfer(bmRequestType, bRequest, wValue, wIndex, wLength,
                         buffer);
...
/*
 * Submit a control transfer to the usb system and continue with the program
 * flow, i.e. do not wait until the packet has been processed.
 */
void PS4EYECam::submit_controlTransfer(uint8_t bmRequestType, uint8_t bRequest,
                                       uint16_t wValue, uint16_t wIndex,
                                       uint16_t wLength, uint8_t *buffer) {
  libusb_fill_control_setup(buffer, bmRequestType, bRequest, wValue, wIndex,
                            wLength);
  libusb_fill_control_transfer(control_transfer, handle_, buffer, ct_done, NULL,
                               1000);
...
void PS4EYECam::firmware_upload() {
  if (!firmwareisloaded) {
    handle_ = libusb_open_device_with_vid_pid(USBMgr::instance()->usbContext(),
                                              0x05a9, 0x0580);
    if (handle_ == NULL) {
    ...
    if (firmware.is_open()) {
      ...
      for (uint32_t pos = 0; pos < length; pos += CHUNK_SIZE) {
        uint16_t size =
            (CHUNK_SIZE > (length - pos) ? (length - pos) : CHUNK_SIZE);
