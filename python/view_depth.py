...
mapx, mapy = cv2.initUndistortRectifyMap(
    K, dist, None, new_camera_matrix, (w, h), cv2.CV_16SC2
)
...
while 1:
    ...
    precentage = 25280
    min_distance = np.mean(
        np.take(z_values, indices[0:precentage])
    )  # takes the 30% lowest measuerements and gets the average distance from these.
    avg_distance = np.mean(z_values)  # averages all distances
    max_distance = np.mean(
        np.take(z_values, indices[z_values.shape[0] - precentage : z_values.shape[0]])
    )  # takes the 30% highest measuerements and gets the average distance from these.
    ...
    cv2.putText(
        color_depth,
        "minimum: " + str(round(min_distance, 1)),
        (5, 20),
    ...
    cv2.putText(
        color_depth,
        "average: " + str(round(avg_distance, 1)),
        (5, 40),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.5,
        (0, 0, 255),
    ...
    cv2.putText(
        color_depth,
        "maximum: " + str(round(max_distance, 1)),
        (5, 60),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.5,
        (0, 0, 255),
