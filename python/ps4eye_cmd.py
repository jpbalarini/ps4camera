...
#     if setup_len != 8:
#         if setup_len != 0:
#             print("stopping after reading setup of size %d" % (len(setup)))
#         break
#     else:
#         wtype = setup[0]
#         wrequest = setup[1]
#         wvalue = setup[2] + setup[3] * 0x100
#         windex = setup[4] + setup[5] * 0x100
#         dlength = setup[6] + setup[7] * 0x100
#         print(
#             "setupTxn: %02X %02X %04X %04X %04X (%d)"
#             % (wtype, wrequest, wvalue, windex, dlength, dlength)
#         )
#         data = array("B", commands.read(dlength))
#         if len(data) != dlength:
#             print(
#                 "stopping after reading data of size %d, expecting %d"
#                 % (len(data), dlength)
#             )
#             break
#         if wtype == 0xC0:
...
#                         "".join(
#                             [
#                                 "{:02x} ".format(b) if a != b else ".. "
#                                 for a, b in zip(data, dev_data)
#                             ]
#                         ),
#                     )
#                 )
#         else:
#             if wtype != 0x40:
#                 print("unknown type, assuming write")
#             dev_length = dev.ctrl_transfer(wtype, wrequest, wvalue, windex, data)
#             if dev_length != dlength:
#                 print("unexpected write of %d/%d bytes" % (dev_length, dlength))
...
#         data = []
#         try:
#             data = dev.read(0x81, 5571968, 100)
