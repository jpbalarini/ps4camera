...
/** \ingroup dev
 * Masks for the bits of the
 * \ref libusb_ss_usb_device_capability_descriptor::bmAttributes "bmAttributes"
 * field field of the SuperSpeed USB Device Capability descriptor.
 */
 ...
 * stdout, warning and error messages are printed to stderr
 *  - LIBUSB_LOG_LEVEL_DEBUG (4)   : debug and informational messages are
 * printed to stdout, warnings and errors to stderr
 */
...
int LIBUSB_CALL libusb_get_config_descriptor_by_value(
...
static inline void libusb_fill_control_transfer(
  ...
  if (setup)
    transfer->length =
        (int)(LIBUSB_CONTROL_SETUP_SIZE + libusb_le16_to_cpu(setup->wLength));
...
libusb_get_iso_packet_buffer_simple(struct libusb_transfer *transfer,
  ...
  return transfer->buffer +
         ((int)transfer->iso_packet_desc[0].length * _packet);
}
...
static inline int libusb_get_descriptor(libusb_device_handle *dev,
  ...
  return libusb_control_transfer(dev, LIBUSB_ENDPOINT_IN,
                                 ...
                                 (uint16_t)((desc_type << 8) | desc_index), 0,
...
void LIBUSB_CALL libusb_set_pollfd_notifiers(
    libusb_context *ctx, libusb_pollfd_added_cb added_cb,
 ...
 * per libusb_context and it is safe to call
...
typedef enum {
   ...
   * associated with a disconnected device. It is safe to call
 ...
 * libusb_hotplug_deregister_callback() or the supplied callback returns 1 to
 * indicate it is finished processing events.
 *
 ...
 * \param[in] events bitwise or of events that will trigger this callback. See
 * \ref libusb_hotplug_event
 * \param[in] flags hotplug callback flags. See \ref libusb_hotplug_flag
 * \param[in] vendor_id the vendor id to match or \ref LIBUSB_HOTPLUG_MATCH_ANY
 * \param[in] product_id the product id to match or \ref
 * LIBUSB_HOTPLUG_MATCH_ANY
 * \param[in] dev_class the device class to match or \ref
 * LIBUSB_HOTPLUG_MATCH_ANY
 * \param[in] cb_fn the function to be invoked on a matching event/device
 * \param[in] user_data user data to pass to the callback function
 * \param[out] handle pointer to store the handle of the allocated callback (can
 * be NULL)
...
void LIBUSB_CALL libusb_hotplug_deregister_callback(
    libusb_context *ctx, libusb_hotplug_callback_handle handle);
